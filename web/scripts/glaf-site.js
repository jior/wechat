...
GLAF.logic.form = {
    tip: function (data) {
        if (data.statusCode == 200) {
            if (data.message) {
                GLAF.ui.tips.html(data.message, data.url);
            }
            else {
                GLAF.ui.tips.suc(data.message, data.url)
            }
        } else {
            GLAF.ui.tips.err(data.message, data.url)
        }
    },
    validate: function () {
                      ...
                      submitHandler: function (form) { 
                          ...
                          if (typeof KindEditor !== "undefined" && KindEditor.instances) {
                              $.each(KindEditor.instances, function () {
                                  ...
                                  if (r) {
                                      if (v.length == 0) {
                                          var msg = m;
                                          GLAF.ui.tips.info(msg ? msg : "内容不能为空")
                                  ...
                                  if (e) {
                                      ...
                                      if (vv.length <= min || vv.length >= max) {
                                          GLAF.ui.tips.info(em ? em : "内容不能小于{0}且大于{1}".format(min, max));
                                          _sb = false; return false;
                                      }
                          ...
                          if (_sb) { 
                              ...
                              $(form).ajaxSubmit({
                                  ...
                                  success: function (d) {
                                      btn.button('reset');
                                      GLAF.logic.form.tip(d);
                                  },
                                  error: function (d) {
                                      btn.button('reset');
                                      GLAF.ui.tips.info(d.responseText);
...
GLAF.logic.page = {
    ...
    common: function () {
        ...
        if ($talbecheckbox.length > 0) {
            $("table.ajax-checkbox input[type='checkbox']").click(function () {
                ...
                if (f) {
                    ...
                    if (l && l != 0) {
                        var tmp = "最多勾选{0}个";
                        if (ck_num > l) { GLAF.ui.tips.info(tmp.format(l)); return false; }
                    }
                    $.post(url, { "id": id, "ck": 1 }, function () {
                        GLAF.ui.tips.suc("已保存")
                    })
        ...
        $("div.location_box").length > 0 && $("div.location_box").empty().append(GLAF.util.location.locSelection($("div.location_box").attr("data-district")));
    ...
    copy: function () {
        ZeroClipboard.setDefaults({
            moviePath: GLAF.domain.k + "/swf/zeroclipboard.swf"
        });
...
GLAF.util.location = {
    ...
    getLocInfo: function (district) {
        if (!GLAF.util.area) return false;

        var self = GLAF.util.location;
        if (self._loc === false) {
            self._loc = {};
            $.each(GLAF.util.area, function (pid, pinfo) {
    ...
    locSelection: function (district) {
        ...
        p.change((function (_, __, ___) {
            return function () {
                ...
                if (GLAF.util.area[pid]) {
                    $.each(GLAF.util.area[pid].l, function (cid, cinfo) {
        ...
        c.change((function (_, __, ___) {
            return function () {
                ...
                if (GLAF.util.area[pid] && GLAF.util.area[pid].l[cid]) {
                    var r = $.extend({}, GLAF.util.area[pid].l[cid].l);
                    var l = [];
                    $.each(r, function (k, v) {
                        l.push({
                            id: k,
        ...
        j.setLocation = function (dist) {
            ...
            var ddInfo = GLAF.util.location.getLocInfo(dist);
            if (ddInfo !== false) {
...
GLAF.logic.uploadify = {
    ...
    live: function () {
        ...
        $("a.item_close").live("click ", function (e) {
            var _self = GLAF.logic.uploadify;
            var _op = GLAF.logic.uploadify.op; 
            $.fallr('show', {
                buttons: {
                    button1: {
                        text: '确定', danger: true, onclick: function () {
        ...
        _self.op.el.uploadify({
            ...
            'onUploadSuccess': function (file, data, response) {
                ...
                if (json.result !== 'SUCCESS') {
                    GLAF.ui.tips.info(json.message || data);
                    return;
                }
                else {
                	if (typeof GLAF.logic.uploadify.Callback !== "undefined"){
				   GLAF.logic.uploadify.Callback(json.image)
				   }
...
if (!((/www.glaf.com/i.test(window.location)))) GLAF.domain.t = "/static", GLAF.domain.k = "/wechat"; 
$(function () {
    GLAF.logic.page.init();
    GLAF.logic.form.init(); 
})
